...
const Artists: React.FC = () => {
  ...
  const { artists, fetchArtists, loading: storeLoading } = useArtistStore();
  ...
  const handleDeleteArtist = async () => {
    ...
    setModalLoading(true);
    try {
      await artistService.deleteArtist(itemToDelete);
      showToast('删除成功');
  ...
  const openAddModal = () => {
    ...
    setFormData({ name: '', previewUrl: '', tags: [] });
    setIsModalOpen(true);
  };
  ...
  const filteredArtists = artists.filter(artist => 
    ...
    artist.tags.some(t => t.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900">艺术家管理</h2>
          <p className="text-sm text-gray-400">管理收录的艺术家列表、预览图和标签</p>
        </div>
      </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-gray-50/50 border-b border-gray-100">
              ...
              <th className="py-4 px-6 text-sm font-semibold text-gray-500 uppercase tracking-wider">艺术家名称</th>
                ...
                <tr key={artist.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="py-4 px-6">
                    <div className="w-12 h-12 rounded-lg bg-gray-100 flex items-center justify-center overflow-hidden border border-gray-100">
                      {artist.previewUrl ? (
                        <img src={artist.previewUrl} alt={artist.name} className="w-full h-full object-cover" />
                      ) : (
                  ...
                  <td className="py-4 px-6">
                    <span className="font-medium text-gray-900 group-hover:text-blue-600 transition-colors cursor-pointer" onClick={() => openEditModal(artist)}>
                      {artist.name}
                    </span>
                  </td>
                  <td className="py-4 px-6">
                    <div className="flex flex-wrap gap-2">
                      {artist.tags && artist.tags.length > 0 ? artist.tags.map((tag, idx) => (
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-5">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">艺术家名称</label>
                <input 
                  type="text"
            ...
            <div className="flex justify-end space-x-3 p-6 bg-gray-50/50 border-t border-gray-100">
              ...
              >
                ...
                <span>{editingArtist ? '保存更改' : '立即创建'}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <ConfirmModal 
        isOpen={confirmOpen}
        title="删除艺术家"
        message="确定要删除这位艺术家吗？该操作将从系统中永久移除此数据，不可撤销。"
        confirmText="确认删除"
        cancelText="取消"
